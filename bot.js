...
// ======== إعدادات ========
const BOT_TOKEN = process.env.BOT_TOKEN; // اضبطه في Environment Variables على Render
const ROOM_ID = process.env.ROOM_ID;     // اضبطه في Environment Variables على Render
...
function subscribeVip(userId) {
    ...
    users[userId].vipUntil = Date.now() + 7 * 24 * 60 * 60 * 1000; // أسبوع
    saveData();
...
function addToList(url, userId) {
    if (isAdmin(userId) && songList.length > 0) songList.unshift(url); // أعلى أولوية
    else if (isVip(userId) && songList.length > 0) songList.splice(1, 0, url); // بعد الأدمن
    else songList.push(url);
    saveData();
}
...
async function handleHighriseCommand(userId, command, args) {
    ...
    switch (command) {
        ...
        case 'balance':
            sendMessage(userId, `💰 ذهب: ${users[userId].gold}\n🎫 تذاكر: ${users[userId].tickets}\n⭐ VIP: ${isVip(userId) ? 'مفعل' : 'غير مفعل'}\n👑 أدمن: ${isAdmin(userId) ? 'نعم' : 'لا'}`);
            break;
        case 'list':
            sendMessage(userId, `📃 القائمة:\n${songList.length ? songList.join('\n') : 'فارغة'}`);
            break;
        case 'skip': if (!isAdmin(userId)) { sendMessage(userId,'❌ التخطي للأدمن فقط'); break; } playNext(); break;
        case 'stop': if (!isAdmin(userId)) { sendMessage(userId,'❌ الإيقاف للأدمن فقط'); break; } songList=[]; saveData(); sendMessage(userId,'🛑 تم إيقاف التشغيل'); break;
        case 'help':
            sendMessage(userId, `🎶 أوامر البوت:\n!play <رابط>\n!subscribe (10 ذهب = 4 تذاكر)\n!vip (500 ذهب = أسبوع غير محدود ⭐)\n!balance\n!list\n!skip (أدمن)\n!stop (أدمن)`);
            break;
    }
}
...
console.log('🔥 بوت Highrise الاحترافي يعمل بنجاح');
